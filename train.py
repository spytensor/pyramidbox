...
from utils.augmentations import PyramidAugmentation,PyramidNormalAugmentation
...
parser.add_argument('--annoPath', default="./data/annodata.txt", help='Location of wider face')  
parser.add_argument('--gamma', default=0.1, help='dyamical learning rate,lr*gamma,working at stepvalues') 
...
means = (116, 112, 109)  # only support voc now
means1=(0.454, 0.439, 0.428)
stds= (0.226, 0.225, 0.223)
num_classes = 1 + 1
...
def train():
    ...
    dataset = Detection(args.annoPath, PyramidNormalAugmentation(ssd_dim, means1,stds), AnnotationTransform())
    ...
    for iteration in range(args.start_iter, max_iter):
        ...
        # forward
        t1 = time.time()
        #print('loaddata: {} sec.' .format((t1 - t0)))
        out = net(images)
        # backprop
        optimizer.zero_grad()
