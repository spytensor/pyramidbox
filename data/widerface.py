...
class Detection(data.Dataset):
    ...
    def __init__(self, anno_file, transform=None, target_transform=None,
                 dataset_name='crowd counting'):
        self.anno_file = anno_file
        #print(self.anno_file)
        ...
        self.counter = 0
        #self.src_img_dir = "/home/wangzy/wzn/yanjiusheng/head_detect/dataset/VOC/JPEGImages"
        self.src_img_dir = "./data/JPEGImages"
        ...
        for line in open(self.anno_file,'r'):
            ...
            if old_cnt==0:
                continue
            img = Image.open(self.src_img_dir+'/'+line[0])
            ...
            for i in range(old_cnt):
                if int(line[cur+i*5])!=1:
                    continue
                x = int(line[cur+i*5+1])
                y = int(line[cur+i*5+2])
                w = int(line[cur+i*5+3])
                h = int(line[cur+i*5+4])
                temp.append(str(x))
                temp.append(str(y))
                temp.append(str(w))
                temp.append(str(h))
            ...
            temp[0] = str(cnt) 
            #print(temp)           
            self.annotation.append(temp)
            self.ids.append(self.src_img_dir+'/'+filename)
